...
function createPatriciaMerkleTree() {
  // Node types
  const NodeType = {
    BRANCH: 'branch',
    EXTENSION: 'extension',
  ...
  // Utility: Convert string key to nibble array (4-bit values)
  function keyToNibbles(key) {
    ...
    for (let i = 0; i < bytes.length; i++) {
      nibbles.push((bytes[i] >> 4) & 0xF);  // High nibble
      nibbles.push(bytes[i] & 0xF);         // Low nibble
    }
  ...
  // Create a branch node (with 16 branches, one for each hex digit)
  ...
  function insert(key, value) {
    ...
    function insertAtNode(nodeHash, path) {
      ...
      if (node.type === NodeType.BRANCH) {
        ...
        return insertAtNode(node.branches[nibble], remainingPath)
          .then(newChildHash => {
            node.branches[nibble] = newChildHash;
            return hashNode(node);
          });
      }
    ...
    return insertAtNode(root, nibbles)
      .then(newRoot => {
        root = newRoot;
        return root;
      });
  ...
  function get(key) {
    ...
    return getFromNode(root, nibbles);
  }
  ...
  function generateProof(key) {
    ...
    return collectProof(root, nibbles)
      .then(exists => {
  ...
  function verifyProof(rootHash, key, value, proof) {
    ...
    return Promise.all(hashPromises)
      .then(() => {
        ...
        return verifyPath(rootHash, nibbles);
      })
      .catch(error => {
